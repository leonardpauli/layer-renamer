...
export default ({
	reuseId, url,
	onLoadFinish, onNativeLog, didClose,
	size = { x: 320, y: 180 }, positionCursorOffset,
} = {})=> {
	positionCursorOffset || (positionCursorOffset = { x: size.x / 2, y: size.y / 2 })
	...
	didClose && (browserWindow.didClose = didClose)
	...
	onLoadFinish && webContents.on('did-finish-load', onLoadFinish)
	onNativeLog && webContents.on('nativeLog', onNativeLog)

	url && browserWindow.loadURL(url)
